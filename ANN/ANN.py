...
def my_relu(in_value):
    ...
    """
    out_value = tf.where(tf.greater(in_value, 0), in_value, 0)
    return out_value
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.truncated_normal([int(X.shape[1]), layersize], stddev=0.05))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    :return: The following variables should be returned in the following order.
        ...
        batch_loss: The average cross-entropy loss of the batch
    """
    w1 = tf.Variable(tf.truncated_normal([int(X.shape[1]), hiddensize], stddev=0.05))
    b1 = tf.Variable(tf.zeros([hiddensize]))
    logits_1 = tf.nn.relu(tf.matmul(X, w1) + b1)
    w2 = tf.Variable(tf.truncated_normal([hiddensize, outputsize], stddev=0.05))
    b2 = tf.Variable(tf.zeros([outputsize]))
    logits = tf.matmul(logits_1, w2) + b2
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=logits)
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    conv1 = tf.layers.conv2d(inputs = X, filters = convlayer_sizes[0],
                             kernel_size = filter_shape[0], padding = padding,
                             activation = tf.nn.relu)
    conv2 = tf.layers.conv2d(inputs = conv1, filters = convlayer_sizes[1],
                             kernel_size = filter_shape[1], padding = padding,
                             activation=tf.nn.relu)
    output = tf.reshape(conv2, shape = [-1, int(conv2.shape[1]) * int(conv2.shape[2]) * convlayer_sizes[1]])
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(output, Y)
